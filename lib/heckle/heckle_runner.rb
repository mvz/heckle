...
require_relative 'minitest_heckler'
    ...
    def self.parse_args argv
      ...
      OptionParser.new do |opts|
        ...
        opts.on(      "--assignments", "Only mutate assignments") do
          ...
          options[:nodes] = Heckler::ASGN_NODES
        end
        ...
        opts.on("-n", "--nodes NODES", "Nodes to mutate",
                "Possible values: #{Heckler::MUTATABLE_NODES.join(',')}") do |opt|
      ...
      Heckler.debug = options[:debug]
      Heckler.timeout = options[:timeout]
